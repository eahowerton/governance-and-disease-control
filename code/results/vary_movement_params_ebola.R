...
test_params <- sens_analysis_setup(change_params = ebol_mod_details$params[c("m1", "m2")],
                                   multiplier = 10,
                                   id_col = c("m1", "m2", "base"))
# add third case where we increase both by ten fold
test_params <- test_params %>% 
  bind_rows(data.frame(m1 = rep(unlist(ebol_mod_details$params["m1"])*10,2), 
                       m2 = rep(unlist(ebol_mod_details$params["m2"])*10,2), 
                       id = rep("m1_m2",2), 
